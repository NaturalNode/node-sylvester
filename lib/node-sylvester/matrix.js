...
Matrix.prototype = {
    ...
    partialPivot: function(k, j, P) {
	var maxIndex = k;
	var maxValue = 0;

	for(var i = k; i <= this.rows(); i++) {
	    if(Math.abs(this.e(i, j)) > maxValue) {
		maxValue = Math.abs(this.e(k, j));
		maxIndex = i;
	    }
	}

	if(maxIndex != k) {
	    var tmp = this.elements[k - 1];
	    this.elements[k - 1] = this.elements[maxIndex - 1];
	    this.elements[maxIndex - 1] = tmp;
...
function luJs() {
    var A = this.dup();
    var L = Matrix.I(A.rows()).slice();
    var P = Matrix.I(A.rows(), A.cols());
    var U = Matrix.Zeros(A.cols(), A.cols());

    console.log(A);

    for(var k = 1; k < A.cols(); k++) {
	P = A.partialPivot(k, k, P);
	
	for(var j = k + 1; j <= A.cols(); j++) {
	    var i = k;
	    var l = A.e(j, i) / A.e(k, i);
	    L.elements[j - 1][i - 1] = l;
	    
	    for(var c = i; c <= A.cols(); c++) {
		U.elements[j - 1][c - 1] = 1;
		A.elements[j - 1][c - 1] -= A.e(k, c) * l;
	    }
	}
    }
